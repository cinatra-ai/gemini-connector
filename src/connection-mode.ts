// The Gemini connector's API-vs-local-CLI connection mode (cinatra#1926,
// epic #1873 M5).
//
...
const CONNECTION_MODE_STORE_KEY = "gemini-connection-mode";
...
export function getPersistedConnectionMode(): ConnectionMode | undefined {
  const stored = getGeminiDeps().readConnectorConfigFromDatabase<StoredConnectionMode>(
...
export function saveConnectionMode(mode: ConnectionMode): void {
  getGeminiDeps().writeConnectorConfigToDatabase(CONNECTION_MODE_STORE_KEY, {
    mode,
 ...
 * reaches to dispatch `@gemini`. Reads the persisted mode and applies the
...
export function getResolvedConnectionTransport(): ConnectionMode {
  return resolveConnectionTransport(
    ...
    getGeminiDeps().localCliEligible(),
  );
}
