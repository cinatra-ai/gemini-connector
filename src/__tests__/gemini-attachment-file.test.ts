...
// functions (not the host `llm-provider-surface` capability). Mock them so the
// adapter's `createClient` builds headers without a live host/deps binding.
vi.mock("../index", () => ({
...
import { geminiUserParts } from "../adapter/adapter-floor";
