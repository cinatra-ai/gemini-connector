...
// LLM provider adapter relocation (llm-providers S4 — cinatra#1715): the Gemini
// request-translation adapter now lives IN the gemini-connector. Its request-
// header builder and telemetry log writer are the connector's OWN
// `buildGeminiRequestHeaders` / `writeGeminiLogFile` (the host round-trip
// through the `llm-provider-surface` capability is gone — the whole adapter is
// connector-local now). Zero behavior change: the connector's self-client
// headers, logging-enabled check + redaction still apply unchanged.
import { buildGeminiRequestHeaders, writeGeminiLogFile } from "../index";
import type {
  LlmProviderAdapter,
  LlmTool,
  LlmFunctionTool,
...
// provider-parts gemini builder, the sandbox-execute tool name, the
// batch-not-supported error) are inlined connector-side in `./adapter-floor`
// — PR-0 moved the TYPES to the ABI leaf; the connector supplies the VALUES.
import {
  geminiUserParts,
  resolvedAttachmentsPerMessage,
} from "./adapter-floor";
import { BatchNotSupportedError } from "./adapter-floor";
...
// ---------------------------------------------------------------------------
// Connector-local members
// ---------------------------------------------------------------------------
//
// The adapter's request-header builder (`buildGeminiRequestHeaders`) and its
// request/response log writer (`writeGeminiLogFile`) are the connector's OWN
// functions (imported from `../index`) now that the adapter is connector-local
// (llm-providers S4 — cinatra#1715): the self-client headers + logging-enabled
// check + redaction all apply unchanged, and a missing `buildRequestHeaders`
// still fails loud connector-side. The host round-trip through the
// `llm-provider-surface` capability is gone. Connection resolution
// (`getConfiguredGeminiAPIKey`) is likewise the connector's OWN function — the
// registry builds the adapter via `createAdapter()` in `../register.ts`, which
// resolves the connector-local API key directly and passes it to the factory
// below.
...
// ---------------------------------------------------------------------------
//
// `getConfiguredGeminiConnection` / `getConfiguredGeminiAPIKey` are the
// connector's OWN functions (see `../index`) — the registry builds the adapter
// via `createAdapter()` in `../register.ts`, which resolves the connector-local
// API key directly and passes it to `createGeminiProviderAdapter(apiKey)`. The
// host round-trip through the `llm-provider-surface` capability is gone now that
// the adapter is connector-local (llm-providers S4 — cinatra#1715).
